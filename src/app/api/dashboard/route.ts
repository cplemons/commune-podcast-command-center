...
async function fetchPodcast(megaphoneApiKey?: string) {
        ...
        return {
                  ...
                  status: { connected: false, error: 'No API key available' },
        };
}

// Instagram (RapidAPI) - uses Posts endpoint for full engagement data
async function fetchInstagram() {
        ...
        try {
          ...
          const profileRes = await fetch(
                      `https://${igHost}/ig_get_fb_profile_hover.php?username_or_url=jeffkrasno`,
                { headers: igHeaders, cache: 'no-store' }
                    );
                  if (!profileRes.ok) throw new Error(`Instagram API error: ${profileRes.status}`);
                  const profileData = await profileRes.json();
                  const user = profileData?.user_data || profileData?.data?.user || profileData?.data || profileData;
                  const followers = Number(user?.follower_count || user?.followers || 0);
                  const postsCount = Number(user?.media_count || user?.posts_count || 0);

          // Get posts with engagement data using the user posts endpoint
          // Try Account Data V2 which returns posts with like/comment counts
          let rawPosts: any[] = [];
                  const postsEndpoints = [
                              `https://${igHost}/ig_get_user_posts.php?username=jeffkrasno`,
                              `https://${igHost}/ig_get_user_posts_v2.php?username=jeffkrasno`,
                            ];

          for (const endpoint of postsEndpoints) {
                      try {
                                    const r = await fetch(endpoint, { headers: igHeaders, cache: 'no-store' });
                                    if (!r.ok) continue;
                                    const d = await r.json();
                                    // Try various response shapes
                        const posts = d?.data || d?.posts || d?.items || d?.user_posts ||
                                        d?.data?.user?.edge_owner_to_timeline_media?.edges || d?.edges || [];
                                    const arr = Array.isArray(posts) ? posts : [];
                                    if (arr.length > 0) { rawPosts = arr; break; }
                      } catch (_) {}
          }

          // If dedicated posts endpoint didn't work, use posts from profile call
          if (rawPosts.length === 0) {
                      const fallbackPosts = profileData?.user_posts || profileData?.posts || [];
                      rawPosts = Array.isArray(fallbackPosts) ? fallbackPosts : [];
          }

          const topPosts = rawPosts.map((p: any) => {
                      const node = p?.node || p;
                      // Views
                                              const views = Number(node?.video_view_count || node?.view_count || node?.play_count || node?.video_play_count || 0);
                      // Likes
                                              const likes = Number(
                                                            node?.like_count ||
                      ...
                      const thumbnail = imgCandidates[0]?.url || node?.display_url || node?.thumbnail_src || node?.thumbnail_url || '';
                      // Caption
                                              const caption = node?.caption?.text || node?.edge_media_to_caption?.edges?.[0]?.node?.text || node?.accessibility_caption || '';
                      // Date
                                              const takenAt = node?.taken_at || node?.taken_at_timestamp || node?.date;
                  ...
                  // Sort by views desc; if views all zero, sort by likes desc
          .sort((a: any, b: any) => {
          ...
          const totalViews = topPosts.reduce((s: number, p: any) => s + (p.views || 0), 0);
                  const avgEngagement = topPosts.length > 0
                    ? topPosts.reduce((s: number, p: any) => s + p.engagementRate, 0) / topPosts.length : 0;
...
async function fetchTikTok() {
        ...
        try {
                              ...
                              if (postsRes.ok) {
                                            ...
                                            topPosts = (Array.isArray(videos) ? videos : []).map((v: any) => {
                                                            ...
                                                            return {
                                                                              ...
                                                                              thumbnail: thumbnail ? `/api/proxy/image?url=${encodeURIComponent(thumbnail)}` : '',
                                                                              views: plays, likes, comments, shares, engagementRate,
                  ...
                  return {
                              profileStats: { followers, following, totalViews, avgEngagement, avgCtr: 0, videoCount, totalLikes },
                              topPosts,
                              status: { connected: true },
                  };
...
// Facebook (RapidAPI) - multi-endpoint fallback
async function fetchFacebook() {
        ...
        for (const url of attempts) {
                  try {
                              ...
                              lastError = `Empty posts from ${url}: keys=${Object.keys(json).join(',')}`;
                  } catch (e: any) { lastError = e.message; }
        }
        if (rawPosts.length === 0) {
                  return { status: { connected: false, error: `Facebook: no data. Last: ${lastError}` } };
        }
        ...
        const topPosts = rawPosts.map((p: any) => {
                  ...
                  return {
                              ...
                              thumbnail: (thumbnail && !isBadUrl) ? `/api/proxy/image?url=${encodeURIComponent(thumbnail)}` : '',
                              likes, comments, shares, reach, engagementScore,
