...
// Instagram (RapidAPI) - fixed field mapping + sort by views desc, fallback to likes
async function fetchInstagram() {
      ...
      try {
              ...
              const user = data?.user_data || data?.data?.user || data?.data || data;
              const rawPosts = data?.user_posts || data?.data?.user?.edge_owner_to_timeline_media?.edges || data?.posts || [];

        const followers = user?.follower_count || user?.followers || user?.edge_followed_by?.count || 0;
              const postsCount = user?.media_count || user?.posts_count || 0;

        const topPosts = rawPosts.map((p: any) => {
                  // Handle both {node: {...}} wrapper and flat post objects
                                            const node = p?.node || p;

                                            // Extract view count - try all known field names
                                            const views = Number(
                                                        node?.video_view_count ||
                                                        node?.view_count ||
                                                        node?.play_count ||
                                                        node?.video_play_count ||
                                                        node?.clips_metadata?.original_sound_info?.ig_artist?.follower_count || // not views, skip
                                                        0
                                                      );

                                            // Extract likes - try all known field names
                                            const likes = Number(
                                                        node?.like_count ||
                                            ...
                                            // Thumbnail - pick best resolution
              ...
              // Sort by views desc; if all views are 0, fall back to likes desc
        .sort((a: any, b: any) => {
...
// Facebook (RapidAPI) - try multiple page identifier formats
async function fetchFacebook() {
  ...
  // Try multiple endpoint + identifier combinations until we get real data
  const attempts = [
  ...
  for (const url of attempts) {
          try {
                    ...
                    // If we got a response but no posts array, check if the response itself is an array
  ...
  if (rawPosts.length === 0) {
          return { status: { connected: false, error: `Facebook: no data returned. Last error: ${lastError}` } };
  }
