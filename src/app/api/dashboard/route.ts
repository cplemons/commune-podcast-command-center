    ...
    if (hasInlineDownloads) {
      ...
      for (let i = 0; i < apiEpisodes.length; i += BATCH) {
        ...
        const batchResults = await Promise.all(batch.map(async (ep: any) => {
          ...
          let streams = 0;
          try {
            // Try /downloads endpoint first (correct Megaphone path), fall back to /analytics
            const dlRes = await fetch(
              `https://cms.megaphone.fm/api/networks/${NETWORK_ID}/podcasts/${PODCAST_ID}/episodes/${ep.id}/downloads`,
              { headers: { 'Authorization': `Token token=${apiToken}` }, cache: 'no-store' }
            );
            const dlStatus = dlRes.status;
            if (dlRes.ok) {
              const ad = await dlRes.json();
              if (Array.isArray(ad)) {
                downloads = ad.reduce((s: number, row: any) => s + Number(row?.downloads || row?.cleanDownloads || row?.total_downloads || row?.count || 0), 0);
                streams = ad.reduce((s: number, row: any) => s + Number(row?.streams || row?.total_streams || 0), 0);
              } else {
                downloads = Number(ad?.downloads || ad?.total_downloads || ad?.cleanDownloads || ad?.clean_downloads || ad?.count || 0);
                streams = Number(ad?.streams || ad?.total_streams || 0);
              }
            } else {
              // /downloads failed -- try /analytics as fallback
              ...
              if (analyticsRes.ok) {
                ...
                // Both failed -- store debug info on first episode only to surface in response
                if (i === 0) {
                  let bodyPreview = '';
                  try { bodyPreview = JSON.stringify(await analyticsRes.json()).substring(0, 200); } catch {}
                  (ep as any).__analyticsDebug = `downloads:${dlStatus} analytics:${analyticsRes.status} body:${bodyPreview}`;
                }
              }
            }
          } catch (e: any) {
            if (i === 0) (ep as any).__analyticsDebug = `exception: ${e.message}`;
          }
          return {
            id: ep.id || ep.uid,
            title: ep.title,
            ...
            delivered: downloads + streams,
            performanceScore: downloads + streams,
            _analyticsDebug: (ep as any).__analyticsDebug || undefined,
          };
        }));
...
async function fetchInstagram() {
          ...
          try {
            ...
            // Debug: capture keys of first raw item to surface in response
            const firstItemKeys = rawItems.length > 0
              ? JSON.stringify(Object.keys(rawItems[0]?.node || rawItems[0] || {}))
              : 'no items';
            const firstItemSample = rawItems.length > 0
              ? JSON.stringify(rawItems[0]?.node || rawItems[0] || {}).substring(0, 300)
              : '';
            ...
            return {
              ...
              topPosts,
              status: { connected: true },
              _debug: { firstItemKeys, firstItemSample, rawItemCount: rawItems.length, postsContainerKeys: Object.keys(postsContainer || {}) },
            };
...
async function fetchFacebook() {
  ...
  try {
    ...
    if (rawPosts.length === 0) {
      // Surface full debug: top-level keys, value types, and first item of any array found
      const debugInfo: any = { topLevelKeys: Object.keys(json || {}) };
      for (const key of Object.keys(json || {})) {
        const val = json[key];
        if (Array.isArray(val)) {
          debugInfo[`${key}_length`] = val.length;
          if (val[0]) debugInfo[`${key}_firstItemKeys`] = Object.keys(val[0]);
        } else if (val && typeof val === 'object') {
          debugInfo[`${key}_keys`] = Object.keys(val);
          for (const subKey of Object.keys(val)) {
            if (Array.isArray(val[subKey])) {
              debugInfo[`${key}.${subKey}_length`] = val[subKey].length;
              if (val[subKey][0]) debugInfo[`${key}.${subKey}_firstItemKeys`] = Object.keys(val[subKey][0]);
            }
          }
        } else {
          debugInfo[`${key}_value`] = String(val).substring(0, 100);
        }
      }
      return { status: { connected: false, error: `No posts found. Debug: ${JSON.stringify(debugInfo)}` } };
    }
