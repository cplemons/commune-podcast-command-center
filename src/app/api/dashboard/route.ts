...
async function fetchPodcast(megaphoneApiKey?: string) {
  ...
  if (!apiToken) {
    return {
      ...
      totalDownloads: null,
      totalStreams: null,
      status: { connected: false, error: 'MEGAPHONE_API_TOKEN not configured' },
    };
  }
  ...
  try {
    ...
    if (hasInlineDownloads) {
      episodes = apiEpisodes.map((ep: any) => {
        ...
        const downloads = Number(
          ep.cleanDownloads || ep.clean_downloads || ep.downloads || ep.total_downloads || 0
        );
      ...
      for (let i = 0; i < apiEpisodes.length; i += BATCH_SIZE) {
        const batch = apiEpisodes.slice(i, i + BATCH_SIZE);
            ...
            try {
              ...
              if (analyticsRes.ok) {
                ...
                if (Array.isArray(analyticsData)) {
                  downloads = analyticsData.reduce(
                    (s: number, row: any) =>
                      s +
                      Number(
                        row?.downloads ||
                  ...
                  streams = analyticsData.reduce(
                    (s: number, row: any) =>
                      s + Number(row?.streams || row?.total_streams || 0),
                    0
                  );
                } else {
                  downloads = Number(
                    analyticsData?.downloads ||
                      analyticsData?.cleanDownloads ||
                      analyticsData?.clean_downloads ||
                      analyticsData?.total_downloads ||
                      0
                  );

                  streams = Number(
                    analyticsData?.streams || analyticsData?.total_streams || 0
                  );
                }
              }
            } catch {
    ...
    return {
      ...
      totalDownloads: null,
      totalStreams: null,
      status: { connected: false, error: apiErr.message },
    };
  }
}
...
async function fetchInstagram() {
  ...
  if (!rapidApiKey) {
    return { status: { connected: false, error: 'RAPIDAPI_KEY not configured' } };
  }
  ...
  try {
    ...
    if (!profileRes.ok) {
      throw new Error(`Instagram profile API error: ${profileRes.status}`);
    }
    ...
    const postsRes = await fetch(`https://${host}/v1/posts?user_id=${userId}`, {
      headers,
      cache: 'no-store',
    });

    if (!postsRes.ok) {
      throw new Error(`Instagram posts API error: ${postsRes.status}`);
    }
    ...
    const topPosts = rawItems
      .map((p: any) => {
        ...
        const imgCandidates =
          node?.image_versions2?.candidates || node?.display_resources || [];
    ...
    const avgEngagement =
      topPosts.length > 0
        ? topPosts.reduce((s: number, p: any) => s + p.engagementRate, 0) /
          topPosts.length
        : 0;
    ...
    return {
      status: {
        connected: false,
        error: `Instagram scraper error: ${err.message}`,
      },
    };
  }
}
...
async function fetchTikTok() {
  ...
  try {
      ...
      if (postsRes.ok) {
        ...
        topPosts = (Array.isArray(videos) ? videos : [])
          .map((v: any) => {
            ...
            const thumbnail =
              v?.video?.cover || v?.cover || v?.thumbnail || v?.originCover || '';
    ...
    return {
      status: {
        connected: false,
        error: `TikTok scraper error: ${err.message}`,
      },
    };
  }
}
  ...
  try {
    ...
    if (rawPosts.length === 0) {
      return {
        status: {
          connected: false,
          error: `No Facebook posts in response. Keys: ${Object.keys(json || {}).join(', ')}`,
        },
      };
    }
    ...
    const avgEngagement =
      topPosts.length > 0 ? totalEngagement / topPosts.length : 0;
    ...
    return {
      status: {
        connected: false,
        error: `Facebook error: ${err.message}`,
      },
    };
  }
}
